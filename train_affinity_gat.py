...
class AffinityGAT(torch.nn.Module):
    ...
    def forward(self, x: Type[torch.tensor], edge_index: Type[torch.tensor], batch: Type[torch.tensor]):
...
def load_dataloaders(save_dir: Path, batch_size: int) -> Tuple[Type[DataLoader]]:
    """
    loads torch geometric dataloaders
    """
    train_ds = BindingDataset(save_dir=save_dir.joinpath("train"))
    val_ds = BindingDataset(save_dir=save_dir.joinpath("val"))
    test_ds = BindingDataset(save_dir=save_dir.joinpath("test"))
...
def eval(
    ...
    """
    evaluates a model on a validation dataset and 
...
def train(
    ...
    """
    trains a model to predict binding affinity between
    ...
    for epoch_idx in range(num_epochs):
        ...
        mse, r2 = eval(model=model, val_dl=val_dl, device=device, loss_fn=loss_fn)
        if mse < lowest_loss:
            lowest_loss = mse
            patience_counter = 0
...
def main():
    ...
    train_dl, val_dl, test_dl = load_dataloaders(save_dir=args.save_dir, batch_size=args.batch_size)
    ...
    test_mse, test_r2 = eval(model=model, val_dl=test_dl, loss_fn=loss_fn, device=device)
